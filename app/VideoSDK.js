...
///// This below code is the one that works but reloads useeffect repeatedly
...
/////Code below is Sally , something wrong with sending to calee and recieving
// import React, { useState, useEffect, useRef } from 'react';
// import {
//   SafeAreaView,
//   TouchableOpacity,
...
//       const { meetingId } = Linking.parse(event.url).queryParams;
//       if (meetingId) {
//         setMeetingId(meetingId);
//         setAutoJoin(true);
//       }
...
//           setMeetingId(meetingId);
//           setAutoJoin(true);
//           console.log( "I am grabbing callee meeting id the caller sent". meetingId);
//           Linking.openURL(`app-garden-loft://VideoSDK?meetingId=${meetingId}`);
//         } else if (response.actionIdentifier === 'DECLINE_CALL') {
//           // Handle decline action if needed
//           //send notification back that "callee is unavailiable"
//         }
//       });
...
import React, { useState, useEffect, useRef } from 'react';
import { SafeAreaView, TouchableOpacity, Text, TextInput, View, FlatList, Dimensions, Platform, Alert } from 'react-native';
...
export default function VideoSDK() {
  ...
  const [autoJoin, setAutoJoin] = useState(false);
  const params = useLocalSearchParams();
  console.log("use local params", params);
  const auth = getAuth();
  const user = auth.currentUser;
  const router = useRouter();
    ...
    if (user) {
      ...
      responseListener.current = Notifications.addNotificationResponseReceivedListener(response => {
        ...
        if (response.actionIdentifier === 'ACCEPT_CALL') {
          setMeetingId(meetingId);
          setAutoJoin(true);
          console.log('Setting callee meeting id with:', meetingId);
          Linking.openURL(`app-garden-loft://VideoSDK?meetingId=${meetingId}`);
        } else if (response.actionIdentifier === 'DECLINE_CALL') {
          // Handle decline action if needed
        }
