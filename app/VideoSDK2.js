...
function ControlsContainer({ join, leave, end, toggleWebcam, toggleMic, addPeople, changeQuality }) {
  const router = useRouter();
...
function MeetingView({ autoJoin, setAutoJoin, callUser, onMeetingEnd, customVideoTrack }) {
  ...
  }, [autoJoin, handleJoin, setAutoJoin]);
...
// below is messi's code trying without setQuality
...
//       {meetingId ? <Text style={{ fontSize: 22, padding: 16 }}>Video Call: {meetingId}</Text> : null}
//       <ParticipantList participants={participantsArrId} />
//       <ControlsContainer end={handleEnd} join={handleJoin} leave={leave} toggleWebcam={toggleWebcam} toggleMic={toggleMic} addPeople={addPeople} />
//       <Modal
//         animationType="slide"
//         transparent={true}
//         visible={modalVisible}
