...
export default function TochTech({ onClose }) {
  ...
  return (
    // Shows Vericare Info pulled from Database by API Doc
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
      <Text style={{ fontSize: 30 }}>Vericare Residents Data</Text>
      {loading ? <ActivityIndicator size="large" /> :

      (
        <>
          <Text style={{ fontSize: 20 }}>Score: {score ?? "N/A"}</Text>
          <Text style={{ fontSize: 26, marginTop: 10 }}>Sleep Quality:</Text>
          {sleepQuality ? (
