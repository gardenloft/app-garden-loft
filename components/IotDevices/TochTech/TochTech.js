...
import { View, Text, Button, ActivityIndicator, Dimensions, TouchableOpacity } from "react-native";
...
export default function TochTech() {
  ...
  useEffect(() => {
    ...
    const nightTimeSleep = sleepQuality ? (sleepQuality.inBedDuration - sleepQuality.wakeDuration) / 60 : 0;
  ...
  return (
    ...
    <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "center", alignItems: "center" }}>
          <View style={{ width: SCREEN_WIDTH * 0.9, backgroundColor: "#fff", padding: 20, borderRadius: 10 }}>
            <Text style={{ fontSize: 22, fontWeight: "bold", textAlign: "center", marginBottom: 10 }}>Sleep Quality</Text>

            {/* Bento Box 1: Sleep Quality Score */}
            <View style={{ backgroundColor: "#f3b718", padding: 15, borderRadius: 10, marginBottom: 10 }}>
              <Text style={{ fontSize: 18, fontWeight: "bold", color: "#000" }}>Sleep Score</Text>
              <Text style={{ fontSize: 22 }}>{score ?? "N/A"}</Text>
            </View>

            {/* Bento Box 2: Night Time Sleep */}
            <View style={{ backgroundColor: "#f09030", padding: 15, borderRadius: 10, marginBottom: 20 }}>
              <Text style={{ fontSize: 18, fontWeight: "bold", color: "#000" }}>Night Time Sleep</Text>
              <Text style={{ fontSize: 22 }}>{nightTimeSleep.toFixed(1)} mins</Text>
            </View>

            {/* Bar Chart */}
            {sleepDurations && (
              <>
                <Text style={{ fontSize: 18, fontWeight: "bold", textAlign: "center", marginBottom: 10 }}>Sleep Stages</Text>
                <BarChart
                  data={{
