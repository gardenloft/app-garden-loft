...
const VideoCall = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (authenticatedUser) => {
      if (authenticatedUser) {
        ...
        listenToFriends(authenticatedUser.uid); // Listen to friends in real-time
      } else {
        setUser(null);
      }
    });
  ...
  const listenToFriends = (userId) => {
    const friendsRef = collection(FIRESTORE_DB, `users/${userId}/friends`);
